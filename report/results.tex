...
\newpage
\section{Results}
In our optimization of the model we iterated over the hyperparameters in the order: No. of layers, learning rate, depth of layers and kernel size. For each hyperparameter we adjusted the value and allowed the multi-task model to train for 10 epochs and then evaluate accuracy of both the Q8 secondary structure prediction as well as the relative and absolute solvent accessibility on both the validation and test set.\\
...
Varying the number of layers in the model greatly affects the size of the model and thus also the time taken to train it. Further, adding more layers increases the models capacity. The higher capacity the more complex task can be solved. Too high capacity on the other hand, and the risk of overfitting rises.\citep[p. 110]{goodfellow-et-al-2016}\\
...
Multi-task & 17 & \multicolumn{1}{l|}{71.666\%} & 12 & 71.810\%
\end{tabular}
\end{table}
